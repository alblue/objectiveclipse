...
public class ExpressionEvaluator {
    public static class EvalException extends Exception {
        private final char[] fProblemArg;
    ...
    public static long getChar(char[] tokenImage, int i) throws EvalException {
        ...
        switch (d) {
            ...
            case '6':
            case '7':
                return getNumber(tokenImage, i + 1, tokenImage.length - 1, 8,
                        IProblem.SCANNER_BAD_OCTAL_FORMAT);

            case 'x':
            case 'u':
            case 'U':
                return getNumber(tokenImage, i + 1, tokenImage.length - 1, 16,
    ...
    private final ArrayList<IASTName> fMacrosInDefinedExpressions = new ArrayList<IASTName>();
    ...
    public IASTName[] clearMacrosInDefinedExpression() {
        IASTName[] result = fMacrosInDefinedExpressions.toArray(new IASTName[fMacrosInDefinedExpressions
                .size()]);
    ...
    private long handleDefined() throws EvalException {
        ...
        if (macro != null) {
            fMacrosInDefinedExpressions.add(fLocationMap.encounterDefinedExpression(macro, fTokens
                    .getOffset(), fTokens.getEndOffset()));
        }
    ...
    private long unaryExpression() throws EvalException {
        switch (LA()) {
            ...
            case IToken.t_false:
                consume();
                return 0;
            case ObjCPreprocessor.tDEFINED:
