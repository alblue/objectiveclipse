...
import org.eclipse.cdt.objc.core.internal.parser.scanner.ImageLocationInfo.MacroImageLocationInfo;
import org.eclipse.cdt.objc.core.internal.parser.scanner.ImageLocationInfo.ParameterImageLocationInfo;
import org.eclipse.cdt.objc.core.internal.parser.scanner.MacroDefinitionParser.TokenParameterReference;
...
public class MacroExpander {
     ...
     * Necessary to properly handle recursive expansions and to figure out
     * whether spaces are required during a stringify operation across such
     * boundaries.
     */
    public static final class ExpansionBoundary extends Token {
        ...
        ExpansionBoundary(PreprocessorMacro scope, boolean isStart) {
            super(ObjCPreprocessor.tSCOPE_MARKER, null, 0, 0);
            fMacro = scope;
            fIsStart = isStart;
        }
    ...
    /**
     * Combines a list of tokens with the preprocessor to form the input for
     * macro expansion.
     */
    ...
    static void addSpacemarker(Token l, Token t, TokenList target) {
        if (isNeighborInSource(l, t)) {
            ...
            if (from != to) {
                target.append(new Token(ObjCPreprocessor.tSPACE, l.fSource, from, to));
            }
        }
        target.append(new Token(ObjCPreprocessor.tNOSPACE, null, 0, 0));
    }
    ...
    private final ArrayList<ImageLocationInfo> fImageLocationInfos = new ArrayList<ImageLocationInfo>();
    private final ArrayList<IASTName> fImplicitMacroExpansions = new ArrayList<IASTName>();
    ...
    public ImageLocationInfo[] clearImageLocationInfos() {
        ImageLocationInfo[] result = fImageLocationInfos.toArray(new ImageLocationInfo[fImageLocationInfos
                .size()]);
    ...
    private ImageLocationInfo createImageLocationInfo(Token t) {
        if (fLocationMap != null) {
            ...
            } else if (s instanceof ObjCPreprocessor) {
    ...
    /**
     * Expects that the identifier has been consumed, stores the result in the
     * list provided.
     */
    public TokenList expand(ITokenSequence lexer, final boolean isPPCondition, PreprocessorMacro macro,
            Token identifier, boolean completionMode) throws OffsetLimitReachedException {
        fImplicitMacroExpansions.clear();
        fImageLocationInfos.clear();
    ...
    private TokenList expandAll(TokenSource input,
            IdentityHashMap<PreprocessorMacro, PreprocessorMacro> forbidden,
            boolean protectDefinedConstructs, MacroExpansionTracker tracker)
                ...
                case IToken.tIDENTIFIER:
                    ...
                    // find the left parenthesis
                        ...
                        if (fLocationMap != null) {
                            ...
                            fImplicitMacroExpansions.add(fLocationMap.encounterImplicitMacroExpansion(macro,
                                    info));
                        }
     ...
     * Expands the macro consuming tokens from the input (to read the
     * parameters) and stores the resulting tokens together with boundary
     * markers in the result token list. Returns the last token of the
     * expansion.
     * 
    ...
    private Token expandOne(Token lastConsumed, PreprocessorMacro macro,
            ...
            TokenList result, MacroExpansionTracker tracker) throws OffsetLimitReachedException {
        if (macro.isFunctionStyle()) {
            ...
            for (int i = 0; i < paramCount; i++) {
                ...
                expandedArgs[i] = needExpansion ? expandAll(argInput, forbidden, false, tracker)
                        : EMPTY_TOKEN_LIST;
                if (!needExpansion) {
                    executeScopeMarkers(argInput, forbidden);
                }
    ...
    private void postProcessTokens(TokenList replacement) {
        ...
        for (Token t = replacement.first(); t != null; t = (Token) t.getNext()) {
            switch (t.getType()) {
                case ObjCPreprocessor.tEXPANDED_IDENTIFIER:
    ...
    private void replaceArgs(PreprocessorMacro macro, TokenList[] args, TokenList[] expandedArgs,
            TokenList result) {
        TokenList replacement = clone(macro.getTokens(fDefinitionParser, fLexOptions, this));
