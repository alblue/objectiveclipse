...
/**
 * Utility to parse macro definitions and create the macro objects for the
 * preprocessor.
 * 
 * @since 5.0
 */
public class MacroDefinitionParser {
    ...
    /**
     * Parses the expansion for a macro, the current token must be the first
     * token of the expansion
     * 
     * @since 5.0
     */
    public void parseExpansion(final Lexer lexer, final ILexerLog log, final char[] name,
            final char[][] paramList, TokenList result) throws OffsetLimitReachedException {
            ...
            switch (candidate.getType()) {
                ...
                case IToken.tIDENTIFIER:
                    if (paramList != null) {
                        ...
                        if (idx >= 0) {
                            candidate = new TokenParameterReference(ObjCPreprocessor.tMACRO_PARAMETER, idx,
                                    lexer.getSource(), candidate.getOffset(), candidate.getEndOffset(),
                                    paramList[idx]);
                            needParam = false;
                        } else {
                            if (needParam) {
                                log.handleProblem(IProblem.PREPROCESSOR_MACRO_PASTING_ERROR, name,
                                        fExpansionOffset, candidate.getEndOffset());
                            } else if (CharArrayUtils.equals(Keywords.cVA_ARGS, image)) {
                                log.handleProblem(IProblem.PREPROCESSOR_INVALID_VA_ARGS, null,
                                        fExpansionOffset, candidate.getEndOffset());
                            }
                            needParam = false;
                        }
    ...
    /**
     * Parses an entire macro definition. Name must be the next token of the
     * lexer.
     */
    public ObjectStyleMacro parseMacroDefinition(final Lexer lexer, final ILexerLog log)
        ...
        return new FunctionStyleMacro(nameChars, paramList, fHasVarArgs, fExpansionOffset,
                fExpansionEndOffset, replacement, source);
    }

    /**
     * Parses a macro definition without the replacement. Name must be the next
     * token of the lexer.
     */
    public PreprocessorMacro parseMacroDefinition(final Lexer lexer, final ILexerLog log,
            final char[] replacement) throws InvalidMacroDefinitionException, OffsetLimitReachedException {
        final Token name = parseName(lexer);
    ...
    private Token parseName(final Lexer lexer) throws OffsetLimitReachedException,
            InvalidMacroDefinitionException {
        ...
        if (tt != IToken.tIDENTIFIER) {
            ...
            throw new InvalidMacroDefinitionException(name.getCharImage(), name.getOffset(), name
                    .getEndOffset());
        }
        fNameToken = name;
        return name;
    ...
    private char[][] parseParamList(Lexer lex, final Token name) throws OffsetLimitReachedException,
            InvalidMacroDefinitionException {
        ...
        do {
            ...
            switch (param.getType()) {
                ...
                case IToken.tRPAREN:
                    ...
                    throw new InvalidMacroDefinitionException(name.getCharImage(), name.getOffset(), param
                            .getEndOffset());
                default:
                    throw new InvalidMacroDefinitionException(name.getCharImage(), name.getOffset(), param
                            .getEndOffset());
        ...
        if (next.getType() != IToken.tRPAREN) {
            throw new InvalidMacroDefinitionException(name.getCharImage(), name.getOffset(), next
                    .getEndOffset());
        }
