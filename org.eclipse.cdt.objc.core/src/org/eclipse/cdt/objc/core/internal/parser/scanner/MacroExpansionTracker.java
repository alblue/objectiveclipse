...
public class MacroExpansionTracker {
    public class MacroInfo {
        private final ArrayList<TokenList> fArguments = new ArrayList<TokenList>();
        private final TokenList fMacroCall = new TokenList();
    ...
    private final LinkedList<MacroInfo> fMacroStack = new LinkedList<MacroInfo>();
                ...
                case IToken.tLPAREN:
                    ...
                    if (nesting == 0) {
                        if (pcount < minfo.fArguments.size()) {
                            ...
                            if (p != null) {
                                active = false;
                                if (n != null && n.getType() != IToken.tCOMMA
                                        && n.getType() != IToken.tRPAREN) {
     ...
     * 
     * @param result
     *            the list of tokens after performing partial expansion up to
     *            the step that was tracked.
     * @param endOffset
     *            the end offset of the input that was read from the lexer.
     */
    void finish(TokenList result, int endOffset) {
        final char[] lexInput = fLexer.getInput();
        if (!isDone()) {
            // special case we compute the entire expansion as one step, the
            // result contains the
            // expanded text
    ...
    /**
     * Returns the code as it looks like just before performing the step that
     * was tracked.
     */
    ...
    /**
     * Returns the replacement that represents the change by the step that was
     * tracked.
     */
    ...
    /**
     * The expanded arguments for the function-style macro expansion are
     * reported.
     * 
     * @param tokenList
     *            the expanded argument, or <code>null</code> if it should not
     *            be expanded.
     */
     ...
     * 
     * @param identifier
     *            the identifier token for the macro expansion.
     */
     ...
     * 
     * @param macro
     *            the macro expanded in the requested step.
     * @param replacement
     *            the replacement for the expansion.
     * @param result
     *            a list to store the macro call with the arguments substituted
     *            in.
     */
    public void storeFunctionStyleMacroReplacement(PreprocessorMacro macro, TokenList replacement,
            TokenList result) {
        MacroInfo minfo = fMacroStack.getLast();
     ...
     *            the macro expanded in the requested step.
     * @param identifier
     *            the token that gets replaced.
     * @param replacement
     *            the replacement
     * @param result
     *            a list to store the macro in.
     */
