/*******************************************************************************
 * Copyright (c) 2007, 2009 Wind River Systems, Inc. and others. All rights
...
/**
 * Converts the offsets relative to various contexts to the global sequence
 * number. Also creates and stores objects that are needed to conform with the
 * IAST... interfaces.
 * 
 * @since 5.0
 */
public class LocationMap implements ILocationResolver {
    ...
    private final ArrayList<ASTMacroDefinition> fBuiltinMacros = new ArrayList<ASTMacroDefinition>();
    private final ArrayList<ASTComment> fComments = new ArrayList<ASTComment>();
    private LocationCtx fCurrentContext = null;

    private final ArrayList<ASTPreprocessorNode> fDirectives = new ArrayList<ASTPreprocessorNode>();
    ...
    private final ArrayList<ASTPreprocessorName> fMacroReferences = new ArrayList<ASTPreprocessorName>();

    private final ArrayList<ASTProblem> fProblems = new ArrayList<ASTProblem>();
    private LocationCtxFile fRootContext = null;
    private final List<ISkippedIndexedFilesListener> fSkippedFilesListeners = new ArrayList<ISkippedIndexedFilesListener>();
    ...
    private ASTFileLocation convertFileLocation(IASTFileLocation loc) {
        ...
        int length = 0;
        if (nodeLength > 0) {
            length = getSequenceNumberForFileOffset(fileName, nodeOffset + nodeLength - 1) + 1
                    - sequenceNumber;
            if (length < 0) {
                return null;
            }
    ...
    /**
     * Creates a name representing a macro in a defined-expression. The returned
     * name can be fed into
    ...
    public IASTName encounterDefinedExpression(IMacroBinding macro, int startOffset, int endOffset) {
        ...
        return new ASTMacroReferenceName(null, IASTPreprocessorStatement.MACRO_NAME, startNumber, endNumber,
                macro, null);
    }
    ...
    /**
     * Creates a name representing an implicit macro expansion. The returned
     * name can be fed into
     * {@link #pushMacroExpansion(int, int, int, int, IMacroBinding, IASTName[], ImageLocationInfo[])}
     * 
     * @param macro
     *            the macro that has been expanded
     * @param imageLocationInfo
     *            the image-location for the name of the macro.
     */
    public IASTName encounterImplicitMacroExpansion(IMacroBinding macro, ImageLocationInfo imageLocationInfo) {
        return new ASTMacroReferenceName(null, IASTPreprocessorMacroExpansion.NESTED_EXPANSION_NAME, 0, 0,
                macro, imageLocationInfo);
    }
    ...
    public void encounterPoundElif(int startOffset, int condOffset, int condEndOffset, int endOffset,
            boolean isActive, IASTName[] macrosInDefinedExpression) {
        startOffset = getSequenceNumberForOffset(startOffset);
        condOffset = getSequenceNumberForOffset(condOffset);
        condEndOffset = getSequenceNumberForOffset(condEndOffset);
        // compatible with 4.0: endOffset=
        // getSequenceNumberForOffset(endOffset);
    ...
    public void encounterPoundError(int startOffset, int condOffset, int condEndOffset, int endOffset) {
        ...
        condOffset = getSequenceNumberForOffset(condOffset);
        condEndOffset = getSequenceNumberForOffset(condEndOffset);
        // not using endOffset, compatible with 4.0: endOffset=
        // getSequenceNumberForOffset(endOffset);
        fDirectives.add(new ASTError(fTranslationUnit, startOffset, condOffset, condEndOffset));
    }

    public void encounterPoundIf(int startOffset, int condOffset, int condEndOffset, int endOffset,
            boolean isActive, IASTName[] macrosInDefinedExpression) {
        startOffset = getSequenceNumberForOffset(startOffset);
        condOffset = getSequenceNumberForOffset(condOffset);
        condEndOffset = getSequenceNumberForOffset(condEndOffset);
    ...
    public void encounterPoundIfdef(int startOffset, int condOffset, int condEndOffset, int endOffset,
        ...
        // not using endOffset, compatible with 4.0: endOffset=
        // getSequenceNumberForOffset(endOffset);
        final ASTIfdef ifdef = new ASTIfdef(fTranslationUnit, startOffset, condOffset, condEndOffset, taken,
                macro);
        fDirectives.add(ifdef);
        addMacroReference(ifdef.getMacroReference());
    }

    public void encounterPoundIfndef(int startOffset, int condOffset, int condEndOffset, int endOffset,
            boolean taken, IMacroBinding macro) {
        startOffset = getSequenceNumberForOffset(startOffset);
        condOffset = getSequenceNumberForOffset(condOffset);
        condEndOffset = getSequenceNumberForOffset(condEndOffset);
        // not using endOffset, compatible with 4.0: endOffset=
        // getSequenceNumberForOffset(endOffset);
        final ASTIfndef ifndef = new ASTIfndef(fTranslationUnit, startOffset, condOffset, condEndOffset,
                taken, macro);
     ...
     *            offset in the current context
     * @param name
     *            name of the include without delimiters ("" or <>)
     * @param filename
     *            the filename of the included file
     * @param userInclude
     *            <code>true</code> when specified with double-quotes.
     * @param active
     *            <code>true</code> when include appears in active code.
     */
    public void encounterPoundInclude(int startOffset, int nameOffset, int nameEndOffset, int endOffset,
        ...
        nameEndOffset = getSequenceNumberForOffset(nameEndOffset);
        endOffset = getSequenceNumberForOffset(endOffset);
        fDirectives.add(new ASTInclusionStatement(fTranslationUnit, startOffset, nameOffset, nameEndOffset,
                endOffset, name, filename, userInclude, active, heuristic));
    }
    ...
    public void encounterPoundUndef(IMacroBinding definition, int startOffset, int nameOffset,
        ...
        // not using endOffset, compatible with 4.0: endOffset=
        // getSequenceNumberForOffset(endOffset);
        final ASTUndef undef = new ASTUndef(fTranslationUnit, name, startOffset, nameOffset, nameEndOffset,
                definition, isActive);
    ...
    public void encounterProblem(int id, char[] arg, int offset, int endOffset) {
        offset = getSequenceNumberForOffset(offset);
        endOffset = getSequenceNumberForOffset(endOffset);
        ASTProblem problem = new ASTProblem(fTranslationUnit, IASTTranslationUnit.SCANNER_PROBLEM, id, arg,
                false, offset, endOffset);
                ...
                if (nodeSpec.canContainMatches(expansion)) {
                    ...
                    if (!nodeSpec.requiresClass(IASTPreprocessorMacroExpansion.class)) {
                        ...
                        if (fTranslationUnit != null) {
                            FindNodeByImageLocation visitor = new FindNodeByImageLocation(
                                    ctx.fSequenceNumber, ctx.getSequenceLength(), nodeSpec);
    ...
    /**
     * Returns the filename of the current context. If the context is a
     * macro-expansion the filename of the enclosing file is returned.
     */
    ...
    public IASTImageLocation getImageLocation(int sequenceNumber, int length) {
        ...
        if (loc instanceof IASTFileLocation) {
            ...
            return new ASTImageLocation(IASTImageLocation.REGULAR_CODE, floc.getFileName(), floc
                    .getNodeOffset(), floc.getNodeLength());
        }
    ...
    /**
     * Returns the sequence number corresponding to the offset in the current
     * context.
     * <p>
     * You must insert all child contexts before the given offset before
     * conversion.
     */
     ...
     * 
     * @param locationCtx
     *            the current context, used to check whether caller and location
     *            map are still in sync.
     */
     ...
     * 
     * @param buffer
     *            the buffer containing the content of the inclusion.
     * @param filename
     *            the filename of the included file
     * @param startOffset
     *            offset in the current context.
    ...
    public ILocationCtx pushInclusion(int startOffset, int nameOffset, int nameEndOffset, int endOffset,
        ...
        int endNumber = getSequenceNumberForOffset(endOffset);
        final ASTInclusionStatement inclusionStatement = new ASTInclusionStatement(fTranslationUnit,
                startNumber, nameNumber, nameEndNumber, endNumber, name, filename, userInclude, true,
                heuristic);
     ...
     * 
     * @param nameOffset
     *            offset within the current context where the name for the
     *            macro-expansion starts.
     * @param nameEndOffset
     *            offset within the current context where the name for the
     *            macro-expansion ends.
     * @param endOffset
     *            offset within the current context where the entire
     *            macro-expansion ends.
     * @param macro
     *            the outermost macro that got expanded.
     * @param implicitMacroReferences
     *            an array of implicit macro-expansions.
     * @param imageLocations
     *            an array of image-locations for the new context.
     */
    public ILocationCtx pushMacroExpansion(int nameOffset, int nameEndOffset, int endOffset,
            int contextLength, IMacroBinding macro, IASTName[] implicitMacroReferences,
            ImageLocationInfo[] imageLocations) {
        assert fCurrentContext instanceof LocationCtxContainer;
        ...
        LocationCtxMacroExpansion expansionCtx = new LocationCtxMacroExpansion(this,
                (LocationCtxContainer) fCurrentContext, nameOffset, endOffset, endNumber, contextLength,
                imageLocations, explicitRef);
    ...
    /**
     * Starts an artificial context that can be used to include files without
     * having a source that contains the include directives.
     * 
     * @param buffer
     *            a buffer containing the include directives.
     * @param isMacroFile
     *            whether the context is used for running the preprocessor,
     *            only.
     */
    public ILocationCtx pushPreInclusion(char[] buffer, int offset, boolean isMacroFile) {
        ...
        fCurrentContext = new LocationCtxContainer((LocationCtxContainer) fCurrentContext, buffer, offset,
    ...
    /**
     * The outermost context must be a translation unit. You must call this
     * method exactly once and before creating any other context.
     */
